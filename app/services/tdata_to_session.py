...
from app.services.session_to_tdata import DC_IP_MAP, _ensure_opentele_patched
...
def find_tdata_dirs(base_dir: Path) -> list[Path]:
    """
    Locate all valid tdata directories containing a 'key_datas' file.
    """
    tdata_dirs: set[Path] = set()

    for p in base_dir.rglob("key_datas"):
        if p.is_file():
...
async def convert_tdata_dir_to_sessions(
    ...
    try:
        ...
        LOGGER.error("opentele not installed for tdata to session conversion")
        return []
    ...
    try:
        ...
        LOGGER.warning("Failed converting tdata %s: %s", tdata_dir, exc)
...
async def process_tdata_to_session_conversion(
    ...
    try:
        ...
        LOGGER.error("opentele not installed")
        return TdataToSessionResult(total=0, converted=0, failed=0, output_path=None)
    ...
    with tempfile.TemporaryDirectory(prefix="ftgc_t2s_") as tmp_dir:
        ...
        if input_path.suffix.lower() == ".zip":
            try:
                extract_zip_tdata_safe(input_path, tmp_path)
        ...
        # Pre-count total accounts across all tdata directories
        total_accounts = 0
        for t_dir in tdata_dirs:
            try:
                td = TDesktop(str(t_dir))
                if td.isLoaded():
                    total_accounts += td.accountsCount
            except Exception as exc:  # noqa: BLE001
                LOGGER.debug("Could not pre-count accounts for %s: %s", t_dir, exc)
        ...
        with tempfile.TemporaryDirectory(prefix="ftgc_t2sout_") as work_dir:
            ...
            for t_dir in tdata_dirs:
                sess_list = await convert_tdata_dir_to_sessions(t_dir, work_path)
                converted_sessions.extend(sess_list)

            converted_count = len(converted_sessions)
            failed_count = (
                total_accounts - converted_count
                if total_accounts >= converted_count
                else 0
            )
